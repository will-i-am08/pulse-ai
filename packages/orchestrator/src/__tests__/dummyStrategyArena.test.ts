...
function dummyBrand(over: Partial<Brand> = {}): Brand {
  return {
    ...
    onboarding_state: { status: "complete" } as Brand["onboarding_state"],
    ...
    voice_analysis_state: { status: "ready" } as Brand["voice_analysis_state"],
    ig_user_id: "ig_dummy",
    ...
    facts: {
      niche: "sports & workplace physiotherapy",
      city: "Austin, TX",
      differentiators: ["desk-athlete clinic", "same-week first visits"],
    } as Brand["facts"],
    visual: {} as Brand["visual"],
    icp: {
