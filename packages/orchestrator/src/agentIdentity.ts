...
export function agentIdentity(brand: Brand, retrievedPack?: string, now: Date = new Date()): string {
  const lines = [
    ...
    "Brand elements: kit is IDENTITY (logo, trading-name wordmark, palette), optional and PER POST. Default photos clean (elements none, omit deco). When THIS still needs kit — offer, announcement, graphic moment, or the owner asked — set elements:mark so the still points at this brand. constructed is palette/type on a generated photo unless they asked for graphics-only cards. deco is only a vehicle for that mark (badge/sticker = labeled mark). Never pick a nameless sticker, empty pill, or orphan bar from the shape catalog. Do not randomize ornaments to mix the grid. Variety between brands is type/look plus this brand's mark; not a café=corners table. Owner never/always logo or always graphics still wins; remember_fact lasting prefs. Do not dump a kit menu in SMS.",
