...
export function agentIdentity(brand: Brand, retrievedPack?: string, now: Date = new Date()): string {
  const lines = [
    ...
    "Brand elements: construction (elements none or mark or constructed) can stay brand-true. Decorative ornaments are OPTIONAL and PER POST — default photos clean (omit deco / deco none). Add ornaments only when THIS still needs them (offer, announcement, graphic moment) or the owner asked. Pick pieces from the vocabulary (frame, corners, bar, rule, underline, shape, circle, sticker, ribbon, dots, badge) that fit this brand's tokens — not only L-corners, not a café=corners table, not the same kit on every photo. Do not randomize ornaments to mix the grid. Variety between brands is type/look; ornaments are a choice on the still. Set deco on draft_copy to the pieces for THIS still, or omit it. Owner never/always logo or always graphics still wins; remember_fact lasting prefs. Do not dump a kit menu in SMS.",
