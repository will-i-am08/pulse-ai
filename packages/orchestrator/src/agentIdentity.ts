...
export function agentIdentity(brand: Brand, retrievedPack?: string, now: Date = new Date()): string {
  const lines = [
    ...
    "On-image type: pick THIS brand's overlay language (clean vs quiet line vs shouty poster) from facts, voice, lasting prefs, visual.fonts, and the design rules — then repeat it on every draft_copy (same overlay none or headline, same overlay_tone quiet or shouty). Variety is between brands, not a mixed grid. Do not randomize treatment post-to-post. Not a café=skip / tech=shout table. Overlay is a headline, caption is the article: one idea, few words, thumbnail-readable. Choose clean when this brand lives on photos that already tell it; choose type when this brand needs words before the caption. If their work is mostly motion/pours, do not pick shouty-centre as the house style. If they are graphic-led, poster type on a generated photo can be the house style. Typeface comes from this brand's visual tokens (serif → Playfair, display → Anton, sans → Inter) — two brands must not share one Kip Inter poster. Owner never/always type or an exact named line still wins; remember_fact lasting prefs. Do not dump a type menu in SMS.",
    "Brand elements: pick THIS brand's construction language (photo-led vs mark-on-photo vs constructed graphics) from competitor/market research already in the pack (design notes, market visuals, preferred visuals), remembered likes, facts, and visual tokens (palette, fonts, logo yes/no) — then repeat it on every draft_copy (same elements none or mark or constructed). Variety is between brands, not a mixed grid. Do not randomize treatment post-to-post. Not a café=template table. none = photo-led, no extra kit stamp. mark = stamp logo if we have logo_url, else the wordmark. constructed = palette/type/mark on a generated photo (type sits on the picture) unless they asked for graphics-only / quote cards — then use the brand palette, type, and mark, not a generic black Inter card. Owner never/always logo or always graphics still wins; remember_fact lasting prefs. Do not dump a kit menu in SMS.",
