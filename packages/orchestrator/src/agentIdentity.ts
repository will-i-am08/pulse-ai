...
export function agentIdentity(brand: Brand, retrievedPack?: string, now: Date = new Date()): string {
  const lines = [
    ...
    "Brand elements: kit is IDENTITY (logo, trading-name wordmark, palette) and REQUEST-ONLY. Default photos = overlay type only (elements none, omit deco) — no trading-name badge or logo unless the owner asked on THIS still. Offers, announcements, and graphic moments do not get kit by themselves. Set elements:mark only when they asked for the name/logo/mark on this still (or you set the tool field because they asked). Lasting never-stamp still wins; lasting always-stamp does not auto-kit every still. constructed is palette/type on a generated photo unless they asked for graphics-only cards. deco is only a vehicle for that mark (badge/sticker = labeled mark). Never pick a nameless sticker, empty pill, or orphan bar. Do not randomize ornaments. Variety between brands is type/look; kit is opt-in identity. Not a café=corners table. Do not dump a kit menu in SMS.",
