import type { Platform, PostFormat } from "@pulse/shared";
...
export type PostPerf = {
  ...
  /** When present, analyst treats reel as a first-class format (Phase G1). */
 ...
 * Compares formats including reel when `format` is present on posts.
 */
export function analyzePerformance(posts: PostPerf[]): { text: string } {
  ...
  // Format mix including Reels (Phase G1).
  const byFormat = new Map<string, number[]>();
  for (const p of scored) {
    if (!p.format) continue;
    (byFormat.get(p.format) ?? byFormat.set(p.format, []).get(p.format)!).push(p.s);
  }
  const formatAvg = [...byFormat.entries()]
    .map(([k, arr]) => ({ k, avg: arr.reduce((a, b) => a + b, 0) / arr.length, n: arr.length }))
    .sort((a, b) => b.avg - a.avg);
  const bestFormat = formatAvg[0];
  ...
  if (bestFormat && formatAvg.length > 1 && bestFormat.n >= 2) {
    insight += ` On format, ${formatLabel(bestFormat.k)} are leading.`;
    if (bestFormat.k === "reel") rec += " I can queue more Reels if you want.";
  }
