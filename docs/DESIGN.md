...
Kip’s mark is a hand-drawn cat face in charcoal on a black square. It is quiet on purpose: the cat is there if you look. Do not invert it, recolour it, add a bubble, or swap it for a lettermark.
...
| `apps/web/public/brand/kip-logo-transparent.png` | Same cat, no black field — only if a black tile would collide with a dark photo. |
...
- Keep at least a quarter of the mark’s width as empty space around it.
- Smallest size in UI: **24px**. Below that the whiskers collapse.
- On black, the square disappears and only the cat remains. That is correct.
- On light surfaces the black square is the icon. Do not drop a white stroke around it.

### Don’t

- Don’t invert to a white cat.
- Don’t put it on a colour, gradient, or patterned field.
...
The standard lockup is the cat mark, then the word, 9–10px gap, vertically centred. That is `BrandLockup`.
...
| Token | Hex | Use |
|---|---|---|
| Black | `#000000` | Landing, logo field |
...
- Corners: 8–16px, never a squircle logo.
...
The landing hero is a real photo (`thread-photo.jpg`), full-bleed, with the product thread overlaid. Photography should feel like something a shop would actually post — not stock-AI gloss. The logo sits in the corner; it does not get a colourful overlay.
